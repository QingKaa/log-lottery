...
const {
  ...
  resetDefault,
  delAll, delItem,
  prizeList, currentPrize,
  selectedPrize, submitData,
  changePrizePerson, changePrizeStatus, selectPrize,
  localImageList,
  saveItem } = usePrizeConfig()
const { t } = useI18n()
</script>
...
<template>
  <div>
    ...
    <VueDraggable v-model="prizeList" :animation="150" handle=".handle" class="p-0 m-0">
      <div v-for="item in prizeList" :key="item.id" class="flex items-center justify-center gap-10 py-5"
        ...
        <label class="w-1/2 max-w-xs form-control">
          ...
          <input v-model="item.name" type="text" :placeholder="t('placeHolder.name')"
        ...
        <label class="flex items-center w-1/2 max-w-xs gap-2 form-control">
          ...
          <input type="checkbox" :checked="item.isAll" class="border-solid checkbox checkbox-secondary border"
        ...
        <label class="w-1/2 max-w-xs form-control">
          ...
          <input v-model="item.count" type="number" :placeholder="t('placeHolder.winnerCount')"
            class="w-full max-w-xs p-0 m-0 input-sm input input-bordered" @change="changePrizePerson(item)">
        ...
        <label class="flex items-center w-1/2 max-w-xs gap-2 form-control">
          ...
          <input type="checkbox" :checked="item.isUsed" class="border-solid checkbox checkbox-secondary border"
        ...
        <label class="w-full max-w-xs form-control">
          ...
          <select v-model="item.picture" class="truncate select select-warning select-sm">
            ...
            <option v-for="picItem in localImageList" :key="picItem.id" :title="picItem.name" class="w-full max-w-full"
              :value="picItem">
        ...
        <label v-if="item.separateCount" class="w-full max-w-xs form-control">
          ...
          <div class="flex justify-start w-full h-full" @click="selectPrize(item)">
            <ul v-if="item.separateCount.countList.length"
              class="flex flex-wrap w-full h-full gap-1 p-0 pt-1 m-0 cursor-pointer">
              <li v-for="se in item.separateCount.countList" :key="se.id"
        ...
        <label class="w-full max-w-xs form-control">
          ...
          <div class="flex gap-2">
            <button class="btn btn-success btn-xs" @click="saveItem(item)">保存</button>
            <button class="btn btn-error btn-xs" @click="delItem(item)">{{ t('button.delete') }}</button>
          </div>
        </label>
      </div>
    </VueDraggable>
    <EditSeparateDialog :total-number="selectedPrize?.count" :separated-number="selectedPrize?.separateCount.countList"
